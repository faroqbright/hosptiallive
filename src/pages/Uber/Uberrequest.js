...
export default function Uberrequest() {
  ...
  const onSubmit = (data) => {
    try {
      if (data) {
        // 
        openInNewTab("https://www.uber.com");
        reset();
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (LoginData !== undefined && LoginType === "Sub Admin") {
      for (const element of LoginData) {
        if (element.module_name === "UBER REQUEST") {
          setAccess({
            add: element.can_add,
                ...
                <div className="row">
                  ...
                  <div className="col-6">
                    <ol className="breadcrumb">
                      ...
                      <li className="breadcrumb-item active"> Uber Request</li>
                    </ol>
                  </div>
                </div>
              </div>
